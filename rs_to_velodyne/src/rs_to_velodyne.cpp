...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        ...
        if (std::strcmp("XYZI", argv[1]) == 0) {
            subRobosensePC = nh.subscribe(input_lidar, 1, rsHandler_XYZI);
        } else if (std::strcmp("XYZIRT", argv[1]) == 0) {
            subRobosensePC = nh.subscribe(input_lidar, 1, rsHandler_XYZIRT);
        } else {
            ROS_ERROR(argv[1]);
